...
// ../../../src/agent-service/machine-auth.ts
var machine_auth_exports = {};
__export(machine_auth_exports, {
  MACHINE_PRINCIPAL: () => MACHINE_PRINCIPAL,
  attestMachinePrincipal: () => attestMachinePrincipal,
  createMachineAuth: () => createMachineAuth,
  generateMachineToken: () => generateMachineToken,
  isAttestedMachinePrincipal: () => isAttestedMachinePrincipal
});
function isSha256Hex(value) {
  return /^[0-9a-f]{64}$/.test(value);
}
function createMachineAuth(config) {
  const configured = config.tokenHash !== void 0 && isSha256Hex(config.tokenHash);
  return {
    ...
    authorizeCredential(credential) {
      ...
      const supplied = (0, import_node_crypto4.createHash)("sha256").update(credential).digest();
      const expected = Buffer.from(config.tokenHash, "hex");
      if (!(0, import_node_crypto4.timingSafeEqual)(supplied, expected)) return { ok: false, error: "MACHINE_AUTH_INVALID" };
      return {
        ok: true,
    ...
    authorize(request) {
      if (!configured) return { ok: false, error: "MACHINE_AUTH_DISABLED" };
      const raw = typeof request.headers.get === "function" ? request.headers.get("authorization") : request.headers.authorization;
      const header = Array.isArray(raw) ? raw[0] ?? "" : raw ?? "";
      const match = /^Bearer\s+(.+)$/i.exec(header);
      if (!match) return { ok: false, error: "MACHINE_AUTH_REQUIRED" };
      const supplied = (0, import_node_crypto4.createHash)("sha256").update(match[1]).digest();
      const expected = Buffer.from(config.tokenHash, "hex");
      if (!(0, import_node_crypto4.timingSafeEqual)(supplied, expected)) return { ok: false, error: "MACHINE_AUTH_INVALID" };
      return {
        ok: true,
...
function generateMachineToken() {
  ...
  return { token, tokenHash: (0, import_node_crypto4.createHash)("sha256").update(token).digest("hex") };
}
function attestMachinePrincipal(result) {
  if (!result.ok) return void 0;
  return { [ATTESTED]: true, credentialRef: result.principal.credentialRef };
}
function isAttestedMachinePrincipal(value) {
  return typeof value === "object" && value !== null && ATTESTED in value && value[ATTESTED] === true;
}
var import_node_crypto4, MACHINE_PRINCIPAL, ATTESTED;
var init_machine_auth = __esm({
  "../../../src/agent-service/machine-auth.ts"() {
    "use strict";
    import_node_crypto4 = require("node:crypto");
    MACHINE_PRINCIPAL = "machine:agentcore-runtime";
    ATTESTED = /* @__PURE__ */ Symbol("machine-principal-attested");
  }
});
...
// ../../../src/agent-core/phases.ts
var import_node_fs5 = require("node:fs");
var import_node_crypto5 = require("node:crypto");
var import_node_path5 = __toESM(require("node:path"), 1);
var import_sdk2 = require("@strands-agents/sdk");

// ../../../src/artifacts/build.ts
var import_node_crypto = require("node:crypto");
var NOW = () => (/* @__PURE__ */ new Date()).toISOString();
function sha256(value) {
  return (0, import_node_crypto.createHash)("sha256").update(value).digest("hex");
}
function buildTaskPacket(s) {
  return {
...
function buildReviewFindings(s) {
  const base = {
    hacp_version: "v0.1-draft",
...
function hacpDecisionFor(choice) {
  const mapped = CHOICE_TO_HACP[choice];
  if (!mapped) throw new Error(`UNKNOWN_CHOICE:${choice}`);
  return mapped;
}
function buildHumanDecision(s, d, invocationReceiptDigest, opts) {
...
function buildAgentReport(s, d, consumptionReceiptId, decisionDigest2, opts) {
  ...
  const behaviour = opts?.simulatedWorkspace ? `${b.behaviour} Simulated workspace (fixture scenario): no real repository files were edited.` : b.behaviour;
  const changedFields = opts?.simulatedWorkspace ? { surfaces_changed: b.files } : { files_changed: b.files };
  const verification = opts?.simulatedWorkspace ? {
    ...
    verification_results: [
      { command: "unit suite", outcome: "not_run", summary: `fixture-provided, not executed in this run: ${s.checks.unit}` },
      { command: "production build", outcome: "not_run", summary: `fixture-provided, not executed in this run: ${s.checks.build}` },
      { command: "dependency audit", outcome: "not_run", summary: `fixture-provided, not executed in this run: ${s.checks.audit}` }
    ]
  } : {
  ...
  return {
    ...
    requested_next_human_decision: void 0,
    ...
    evidence_refs: [
      ...
      { ref: `consumption:${consumptionReceiptId}`, kind: "other", summary: `Decision digest sha256:${decisionDigest2}` }
    ],
...
var SCHEMA_DIR = import_node_path.default.resolve(process.cwd(), "schemas/hacp/v0.1-draft");
...
for (const file of Object.values(SCHEMA_FILES).concat("common-defs.schema.json")) {
  const schema = JSON.parse((0, import_node_fs.readFileSync)(import_node_path.default.join(SCHEMA_DIR, file), "utf8"));
  const id = schema.$id;
...
function assertValid(kind, artifact) {
...
// ../../../src/store-admission.ts
var STORE_ADMISSION_SCHEMA = "who-decides.local-store-admission.v1";
var LEGACY_CONSUMPTION_WRITER_VERSION = "0.1.0";
var fail = (detail) => {
  throw new Error(`OWNER_STORE_ADMISSION_FAILED: ${detail}`);
};
var isUnsafePath = (dbPath) => !import_node_path2.default.isAbsolute(dbPath) || dbPath === ":memory:" || dbPath.startsWith("file:") || /[?\0]/.test(dbPath);
var physicalIdentity = (dbPath) => {
  const stat = (0, import_node_fs2.statSync)(dbPath, { bigint: true });
  const fs = (0, import_node_fs2.statfsSync)(dbPath, { bigint: true });
  return { device: stat.dev.toString(), inode: stat.ino.toString(), filesystemType: fs.type.toString() };
};
var schemaFingerprint = (db) => (0, import_node_crypto2.createHash)("sha256").update(JSON.stringify(
  db.prepare("SELECT type,name,tbl_name,sql FROM sqlite_master WHERE name NOT GLOB 'sqlite_*' ORDER BY type,name").all()
)).digest("hex");
var normalizeWriter = (writer) => ({ role: writer.role, version: writer.version, insertionPath: writer.insertionPath });
var sortedWriters = (writers) => writers.map(normalizeWriter).sort((a, b) => a.role.localeCompare(b.role));
var manifestPayload = (admission) => JSON.stringify({
...
var storeAdmissionDigest = (admission) => (0, import_node_crypto2.createHash)("sha256").update(manifestPayload(admission)).digest("hex");
function validateWriterInventory(writers) {
...
function openAdmittedStore(dbPath, admission, writer) {
  if (!admission || admission.schema !== STORE_ADMISSION_SCHEMA || isUnsafePath(dbPath) || !(0, import_node_fs2.existsSync)(dbPath)) fail("normal startup requires an existing pinned absolute database");
  validateWriterInventory(admission.writers);
  const resolved = (0, import_node_fs2.realpathSync)(dbPath);
  ...
  const db = new import_better_sqlite3.default(resolved, { fileMustExist: true });
  try {
...
function verifyAdmittedStore(db, admission) {
  if (!admission || admission.schema !== STORE_ADMISSION_SCHEMA || isUnsafePath(admission.canonicalPath)) fail("invalid store admission");
  validateWriterInventory(admission.writers);
  verifyAdmittedGuardDirectory(admission);
  const main = db.pragma("database_list").find((row2) => row2.name === "main");
  if (!main) throw new Error("OWNER_STORE_ADMISSION_FAILED: opened SQLite main database is missing");
  if ((0, import_node_fs2.realpathSync)(main.file) !== admission.canonicalPath) fail("opened SQLite main database does not match admission");
  ...
  const row = db.prepare("SELECT database_id,config_generation,manifest_sha256 FROM owner_store_admission WHERE singleton=1").get();
  const digest = storeAdmissionDigest(admission);
...
function requireClosedWriterInventory(db, admission) {
  verifyAdmittedStore(db, admission);
  const rows = db.prepare("SELECT role,version,insertion_path,config_generation,manifest_sha256 FROM owner_writer_attestations ORDER BY role").all();
  const digest = storeAdmissionDigest(admission);
...
function verifyAdmittedGuardDirectory(admission) {
  if (!(0, import_node_fs2.existsSync)(admission.guardDirectory) || (0, import_node_fs2.realpathSync)(admission.guardDirectory) !== admission.guardDirectory) fail("canonical guard directory is missing or aliased");
  ...
  if (!(0, import_node_fs2.existsSync)(admission.anchorDirectory) || (0, import_node_fs2.realpathSync)(admission.anchorDirectory) !== admission.anchorDirectory) fail("admission anchor directory is missing or aliased");
  const anchorStat = (0, import_node_fs2.statSync)(admission.anchorDirectory, { bigint: true });
  ...
  const dbIdentityPath = import_node_path2.default.join(admission.anchorDirectory, "db-identity");
  if (!(0, import_node_fs2.existsSync)(dbIdentityPath)) fail("admission anchor hardlink is missing; the original database inode can no longer be proven alive");
  ...
  const generationPath = import_node_path2.default.join(admission.anchorDirectory, "generation");
  if (!(0, import_node_fs2.existsSync)(generationPath)) fail("generation sidecar is missing; the admitted file generation cannot be confirmed");
  ...
  const token = (0, import_node_fs2.readFileSync)(generationPath, "utf8");
  if ((0, import_node_crypto2.createHash)("sha256").update(token).digest("hex") !== admission.generationTokenDigest) fail("generation token does not match the admitted generation");
}
function rejectUnadmittedAccess(db) {
...
// ../../../src/consumption/store.ts
var CONSUMPTION_SCHEMA = "who-decides.consumption-receipt.v0";
function validateExpiry(expiresAt) {
  ...
  if (!match || !Number.isFinite(time) || month < 1 || month > 12 || day < 1 || day > days[month - 1] || Number(match[4]) > 23 || Number(match[5]) > 59 || Number(match[6]) > 59 || Number(match[8] ?? 0) > 23 || Number(match[9] ?? 0) > 59) {
...
function decisionDigest(decision) {
  ...
  return `sha256:${(0, import_node_crypto3.createHash)("sha256").update(canonical).digest("hex")}`;
}
var ConsumptionStore = class {
  db;
  options;
  constructor(dbPath, options = {}) {
    ...
    if (options.admission) {
      ...
      (0, import_node_fs3.mkdirSync)(import_node_path3.default.dirname(dbPath), { recursive: true });
      this.db = new import_better_sqlite32.default(dbPath);
      try {
        rejectUnadmittedAccess(this.db);
      } catch (error) {
        this.db.close();
        throw error;
      }
  ...
  claim(decision, successorInvocationId, expectedDigest) {
    if (this.options.admission) {
      try {
        requireClosedWriterInventory(this.db, this.options.admission);
      } catch {
        return { status: "rejected", reason: "environment_blocked", detail: "owner-admitted writer inventory is unavailable or changed" };
      }
    }
    if (expectedDigest !== void 0 && expectedDigest !== decisionDigest(decision)) {
      return { status: "rejected", reason: "digest_mismatch", detail: "provided digest does not match the decision record" };
    }
    if (decision.expiresAt !== void 0) {
    ...
    try {
      ...
      const receipt = {
        ...
        receiptId: (0, import_node_crypto3.randomUUID)(),
        decisionId: decision.decisionId,
  ...
  readClaim(decisionId) {
    return this.db.prepare("SELECT receipt_json, successor_invocation_id, decision_digest FROM consumption_receipts WHERE decision_id = ?").get(decisionId);
  }
  checkReplay(stored, decision, successor) {
    const receipt = JSON.parse(stored.receipt_json);
    const digest = decisionDigest(decision);
    if (stored.decision_digest !== digest || receipt.decisionDigest !== digest || receipt.decisionId !== decision.decisionId || receipt.decisionRequestId !== decision.decisionRequestId || receipt.permittedAction !== decision.permittedAction || receipt.successorInvocationId !== stored.successor_invocation_id) {
  ...
  getReceipt(decisionId) {
    const row = this.db.prepare("SELECT receipt_json FROM consumption_receipts WHERE decision_id = ?").get(decisionId);
    return row ? JSON.parse(row.receipt_json) : void 0;
  }
  close() {
    this.db.close();
  }
...
// ../../../src/provider.ts
var import_openai = require("@strands-agents/sdk/models/openai");
var import_bedrock = require("@strands-agents/sdk/models/bedrock");
function required(name) {
  const value = process.env[name];
  if (!value) {
    throw new Error(
      `ENVIRONMENT_BLOCKED: ${name} is not set. See .env.example for the provider configuration contract.`
    );
  }
  return value;
}
function loadProvider() {
  const kind = process.env.WD_PROVIDER ?? "openai-compatible";
  if (kind === "openai-compatible") {
    const baseUrl = required("WD_BASE_URL");
    const modelId2 = required("WD_MODEL");
    const apiKey = required("WD_API_KEY");
    const model2 = new import_openai.OpenAIModel({
      api: "chat",
      modelId: modelId2,
      apiKey,
    ...
    return {
      kind,
      model: model2,
      provenance: { provider: kind, modelId: modelId2, baseUrl }
    };
  }
  ...
  const model = new import_bedrock.BedrockModel({ modelId, region });
  return {
    kind,
    model,
...
// ../../../src/agent-core/tool.ts
var import_sdk = require("@strands-agents/sdk");
function decisionTool(f) {
  return new import_sdk.FunctionTool({
...
// ../../../src/agent-core/durable.ts
var import_node_fs4 = require("node:fs");
var import_node_path4 = __toESM(require("node:path"), 1);
function writeDurableJson(filePath, value) {
  const tmp = `${filePath}.tmp-${process.pid}`;
  const fd = (0, import_node_fs4.openSync)(tmp, "w");
  try {
    (0, import_node_fs4.writeSync)(fd, JSON.stringify(value, null, 2));
    (0, import_node_fs4.fsyncSync)(fd);
  } finally {
    (0, import_node_fs4.closeSync)(fd);
  }
  (0, import_node_fs4.renameSync)(tmp, filePath);
  syncDir(import_node_path4.default.dirname(filePath));
}
function syncDir(dir) {
  const dirFd = (0, import_node_fs4.openSync)(dir, "r");
  try {
    (0, import_node_fs4.fsyncSync)(dirFd);
  } finally {
    (0, import_node_fs4.closeSync)(dirFd);
  }
}
function mkdirDurable(dir) {
  const missing = [];
  let current = dir;
  while (!(0, import_node_fs4.existsSync)(current)) {
  ...
  for (const level of missing) syncDir(import_node_path4.default.dirname(level));
}
function reserveExclusive(filePath) {
  let fd;
  try {
    fd = (0, import_node_fs4.openSync)(filePath, "wx");
  } catch (err) {
    if (err.code === "EEXIST") return false;
    throw err;
  }
  try {
    (0, import_node_fs4.writeSync)(fd, JSON.stringify({ holderPid: process.pid, acquiredAt: (/* @__PURE__ */ new Date()).toISOString() }));
    (0, import_node_fs4.fsyncSync)(fd);
  } finally {
    (0, import_node_fs4.closeSync)(fd);
  }
...
function saveRunState(stateFile2, state) {
  writeDurableJson(stateFile2, state);
}
function assertSafeSlug(tag, label) {
...
var ID_KEYS = /* @__PURE__ */ new Set(["packet_id", "target_id", "stop_id", "report_id", "finding_id", "finding_tradeoff_id", "decision_id", "run_id"]);
var defaultRuntimeFactory = (fixture) => {
  const { model, provenance } = loadProvider();
  return { agent: new import_sdk2.Agent({ model, tools: [decisionTool(fixture)] }), provenance };
};
function stateFile(ctx2, tag) {
  return import_node_path5.default.join(ctx2.dataDir, `state-${tag}.json`);
}
function snapshotFile(ctx2, tag) {
  return import_node_path5.default.join(ctx2.dataDir, `snapshot-${tag}.json`);
}
function leaseFile(ctx2, tag) {
  return import_node_path5.default.join(ctx2.dataDir, `lease-${tag}.json`);
}
function runDir(ctx2, tag) {
  return import_node_path5.default.join(ctx2.dataDir, "runs", tag);
}
function loadServiceState(ctx2, tag) {
  const file = stateFile(ctx2, tag);
  if (!(0, import_node_fs5.existsSync)(file)) return null;
  return JSON.parse((0, import_node_fs5.readFileSync)(file, "utf8"));
}
function writeArtifact(ctx2, tag, name, value) {
  writeDurableJson(import_node_path5.default.join(runDir(ctx2, tag), name), value);
}
async function startPhase(ctx2, input, runtimeFactory = defaultRuntimeFactory) {
  assertSafeSlug(input.tag, "TAG");
  const prior = loadServiceState(ctx2, input.tag);
  ...
  if (prior) {
    if (prior.phase === "awaiting_decision") {
      return { status: "DECISION_REQUIRED", decisionId: prior.decisionId, invocationA: prior.invocationA, decisionRequest: decisionRequestOf(ctx2) };
    }
    return { status: "HUMAN_DECISION_REQUIRED", reason: `prior run is ${prior.phase}; automatic recovery is disabled` };
  }
  if ((0, import_node_fs5.existsSync)(snapshotFile(ctx2, input.tag))) {
    return { status: "HUMAN_DECISION_REQUIRED", reason: "snapshot exists without state; unknown outcome, automatic recovery disabled" };
  }
  mkdirDurable(ctx2.dataDir);
  if (!reserveExclusive(leaseFile(ctx2, input.tag))) {
    return { status: "ENVIRONMENT_BLOCKED", reason: "tag is already reserved; holder death does not authorize takeover" };
  }
  mkdirDurable(runDir(ctx2, input.tag));
  const { agent, provenance } = runtimeFactory(ctx2.fixture);
  const invocationA = `inv-${(0, import_node_crypto5.randomUUID)()}`;
  const first = await agent.invoke(promptFor(ctx2.fixture));
  ...
  const expectedPatchId = `${ctx2.fixture.package}-${ctx2.fixture.to_version}`;
  const reason = interrupts[0].reason;
  if (interrupts[0].name !== "human_release_decision" || reason?.question !== ctx2.fixture.decision_request.question || reason?.patchId !== expectedPatchId || JSON.stringify(reason?.options) !== JSON.stringify(ctx2.fixture.decision_request.options)) {
  ...
  writeDurableJson(snapshotFile(ctx2, input.tag), agent.takeSnapshot({ preset: "session" }));
  const sf = scopedFixture(ctx2.fixture, input.tag);
  ...
  writeArtifact(ctx2, input.tag, "01-task-packet.json", packet);
  writeArtifact(ctx2, input.tag, "02-review-findings.json", findings);
  writeArtifact(ctx2, input.tag, "03-stop-response.json", stop);
  syncDir(runDir(ctx2, input.tag));
  const state = {
    ...
    decidedAt: "",
    invocationA,
    invocationB: `inv-${(0, import_node_crypto5.randomUUID)()}`,
    fixtureDigest: (0, import_node_crypto5.createHash)("sha256").update(JSON.stringify(ctx2.fixture)).digest("hex")
  };
  saveRunState(stateFile(ctx2, input.tag), state);
  return { status: "DECISION_REQUIRED", decisionId: state.decisionId, invocationA, decisionRequest: decisionRequestOf(ctx2) };
}
function decisionRequestOf(ctx2) {
  const f = ctx2.fixture;
  return {
    question: f.decision_request.question,
...
async function resumePhase(ctx2, input, runtimeFactory = defaultRuntimeFactory) {
  assertSafeSlug(input.tag, "TAG");
  const prior = loadServiceState(ctx2, input.tag);
  ...
  const f = ctx2.fixture;
  const currentFixtureDigest = (0, import_node_crypto5.createHash)("sha256").update(JSON.stringify(f)).digest("hex");
  ...
  const decisionLock = import_node_path5.default.join(ctx2.dataDir, `decision-lock-${input.tag}.lock`);
  mkdirDurable(ctx2.dataDir);
  ...
  const state = { ...prior, phase: "claimed", choice: input.choice, rationale, decidedAt };
  saveRunState(stateFile(ctx2, input.tag), state);
  ...
  const store = new ConsumptionStore(ctx2.claimDb);
  ...
  if (claim.status === "rejected") {
    ...
    saveRunState(stateFile(ctx2, input.tag), { ...state, phase: "rejected", outcome });
  ...
  const snapshot = JSON.parse((0, import_node_fs5.readFileSync)(snapshotFile(ctx2, input.tag), "utf8"));
  const { agent } = runtimeFactory(f);
  agent.loadSnapshot(snapshot);
  const interruptId = interruptIdFromSnapshot(snapshot);
  const resumed = await agent.invoke([
    new import_sdk2.InterruptResponseContent({ interruptId, response: { choice: state.choice, rationale: state.rationale } })
  ]);
  if (resumed.stopReason !== "endTurn") {
    store.close();
    throw new Error(`INVARIANT: resume should end the turn, got ${resumed.stopReason}`);
  }
  const imposter = `inv-${(0, import_node_crypto5.randomUUID)()}`;
  ...
  const runtimeF = sf;
  const persistedStop = (0, import_node_fs5.readFileSync)(import_node_path5.default.join(runDir(ctx2, input.tag), "03-stop-response.json"), "utf8");
  const evidenceDigest = (0, import_node_crypto5.createHash)("sha256").update(persistedStop).digest("hex");
  const humanDecision = buildHumanDecision(runtimeF, runtimeDecision, evidenceDigest, {
    channel: (await Promise.resolve().then(() => (init_machine_auth(), machine_auth_exports))).isAttestedMachinePrincipal(input.machinePrincipal) ? {
      interaction: "api",
  ...
  const effect = {
    ...
    effect: state.choice,
    mode: "dry-run",
    exactPayload: state.choice === "create_draft_pr" ? { repo: "example/kestrel-app", title: `Security: ${f.package} ${f.to_version}`, branch: `security/${f.package}-${f.to_version}`, payloadSource: "host-constructed from fixture (model output advisory only)" } : { outcome: state.choice === "send_back" ? "no PR created \u2014 work returned" : "nothing executed \u2014 deferred", payloadSource: "host-constructed from fixture (model output advisory only)" },
  ...
  writeArtifact(ctx2, input.tag, "04-human-decision.json", humanDecision);
  writeArtifact(ctx2, input.tag, "05-consumption-receipt.json", claim.receipt);
  writeArtifact(ctx2, input.tag, "06-effect-receipt.json", effect);
  writeArtifact(ctx2, input.tag, "07-agent-report.json", report);
  writeArtifact(ctx2, input.tag, "00-service-run-summary.json", {
    tag: input.tag,
    decisionId: state.decisionId,
    invocationA: state.invocationA,
    invocationB: state.invocationB,
    humanChoice: state.choice,
    receiptId: claim.receipt.receiptId,
    duplicateProbe: "REJECTED (competing_successor)"
  });
  syncDir(runDir(ctx2, input.tag));
  saveRunState(stateFile(ctx2, input.tag), { ...state, phase: "completed", receiptId: claim.receipt.receiptId });
...
function interruptIdFromSnapshot(snapshot) {
...
// ../../../fixtures/patch-scenario.json
var patch_scenario_default = {
  run_id: "run-2026-09-03-001",
  packet_id: "packet-2026-09-03-001",
  decision_id: "decision-2026-09-03-001",
  finding_tradeoff_id: "finding-2026-09-03-002",
  report_id: "report-2026-09-03-001",
  stop_id: "stop-2026-09-03-001",
  package: "kestrel-web",
  from_version: "4.2.0",
  to_version: "4.3.0-security",
  advisory: "KESTREL-2026-0042 (fixture): request smuggling in header parsing",
  tradeoff: {
    kind: "runtime_floor_bump",
    from: "node 18",
    to: "node 20",
    who_is_affected: "Teams still deploying on node 18 LTS cannot take this patch without an engine upgrade",
    unresolved_check: "integration suite not runnable on node 18 after the bump \u2014 one compatibility check open"
  },
  checks: {
    unit: "pass \u2014 214/214",
    build: "pass \u2014 production bundle built",
    audit: "1 known issue carried (pre-existing, unrelated)"
  },
  decision_request: {
    question: "Create the draft PR for kestrel-web 4.3.0-security?",
    options: ["create_draft_pr", "send_back", "defer"],
    human_terms: "Tests green, build green, but the patch raises the runtime floor from node 18 to node 20 \u2014 node 18 teams cannot take it without an engine upgrade, and one node 18 compatibility check is unresolved."
  },
  human_choice: {
    decision: "create_draft_pr",
    rationale: "Security risk outweighs the floor bump; node 18 consumers get a heads-up in the PR description."
  },
  created_by_agent: "who-decides-agent",
  human_operator: "joe"
};

// ../../../src/agent-service/fixture.ts
var patchScenario = patch_scenario_default;

// ../../../src/agent-service/server.ts
var import_meta = {};
var PORT = Number(process.env.WD_AGENT_PORT ?? 8080);
var DATA_DIR = process.env.WD_AGENT_DATA_DIR ?? import_node_path6.default.resolve(process.cwd(), ".tmp/agent-service");
var CLAIM_DB = process.env.WD_AGENT_CLAIM_DB ?? import_node_path6.default.join(DATA_DIR, "consumption.db");
var ctx = { dataDir: DATA_DIR, claimDb: CLAIM_DB, fixture: patchScenario };
var machineAuth = createMachineAuth({ tokenHash: process.env.WD_MACHINE_TOKEN_HASH });
...
var server = (0, import_node_http.createServer)(async (req, res) => {
  ...
  if (req.method === "POST" && req.url === "/invocations") {
    ...
    const bodyCredential = typeof payload.credential === "string" ? payload.credential : void 0;
    const auth = bodyCredential !== void 0 ? machineAuth.authorizeCredential(bodyCredential) : machineAuth.authorize(req);
    if (!auth.ok) {
...
if (process.argv[1] && import_meta.url === new URL(`file://${process.argv[1]}`).href) {
  server.listen(PORT, () => {
...
// main.ts
var PORT2 = Number(process.env.WD_AGENT_PORT ?? 8080);
if (!server.listening) {
  server.listen(PORT2, "0.0.0.0", () => {
    console.log(`[agentcore] who-decides agent listening on 0.0.0.0:${PORT2}`);
  });
}
