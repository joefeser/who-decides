...
test('owner-admitted store rejects aliases drift unsafe paths and incomplete writer inventory',()=>{
  ...
  const guardDrift=make();try{const alternate=path.join(path.dirname(guardDrift.admission.canonicalPath),'alternate-guards');mkdirSync(alternate);assert.throws(()=>new LocalOwnerVerifier(configFor({...guardDrift.admission,guardDirectory:alternate})),/persistent database identity/)}finally{rmSync(guardDrift.dir,{recursive:true})}
